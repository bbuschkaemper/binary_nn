...
DEFAULT_SHADOWFREE_TERNARY_HIDDEN_DIMS = (64,)
DEFAULT_SHADOWFREE_TERNARY_LEARNING_RATE = 3e-3


def train_shadowfree_ternary_regression(
    ...
    *,
    ...
    resolved_training_config = training_config or TrainingConfig(
        hidden_dims=DEFAULT_SHADOWFREE_TERNARY_HIDDEN_DIMS,
        learning_rate=DEFAULT_SHADOWFREE_TERNARY_LEARNING_RATE,
    )
    return train_regression_model(
        model_builder=lambda input_dim, hidden_dims: ShadowFreeTernaryRegressor(
            input_dim=input_dim,
...
def _build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train a shadow-free ternary residual regressor."
    )
    ...
    parser.add_argument(
        "--learning-rate",
        type=float,
        default=DEFAULT_SHADOWFREE_TERNARY_LEARNING_RATE,
    )
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        default=list(DEFAULT_SHADOWFREE_TERNARY_HIDDEN_DIMS),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    result = train_shadowfree_ternary_regression(
